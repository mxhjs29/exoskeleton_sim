...
# initial_qpos
file_path = "/home/chenshuo/PycharmProjects/move_sim/SMPL/initial_state.yaml"
...
# mocap_data
quat_SMPL_base = [1,0,0,0]
kinetic_chain_right = [0,1,2]
...
class EnvCarry_WithoutExoWeld(gym.Env, gym.utils.EzPickle, ObsVecDict):
    DEFAULT_CREDIT = """\
    ...
    def _setup(self,
        ...
        act_low = -np.ones(self.sim.model.nu - 18) if self.normalize_act else self.sim.model.actuator_ctrlrange[:,0].copy()
        act_high = np.ones(self.sim.model.nu - 18) if self.normalize_act else self.sim.model.actuator_ctrlrange[:,1].copy()
        self.action_space = gym.spaces.Box(act_low,act_high,dtype=np.float64)
        self.scale = scale
        self.apply_exter_force = apply_exter_force
    ...
    def step(self,a, **kwargs):
        ...
        if (self.flag_animation < 147 and self.end_flag == 0):
            command = 0
            self.sim.data.ctrl = grasp_handle(self.sim.data.ctrl, self.grap_actuators, self.grap_actuators_id, command,a)
        ...
        force_constraint_r = self.sim.data.qfrc_constraint[self.hip_joints_id['hip_flexion_r']]
        force_constraint_l = self.sim.data.qfrc_constraint[self.hip_joints_id['hip_flexion_l']]
    ...
    def _forward(self,**kwargs):
        ...
        if self.flag_animation >= self.length_data-1:
            self.flag_animation = 0
        # + env_info['rwd_control'] + env_info['rwd_vel']
    ...
    def get_reward_dict(self,obs_dict):
        ...
        knee_error = obs_dict['qpos_error'][:,:,[self.knee_joints_id[name] for name in self.knee_joints]]
        knee_error_reward = -1 * np.linalg.norm(knee_error)
        else_error = np.delete(obs_dict['qpos_error'],[self.knee_joints_id[name] for name in self.knee_joints])
        else_error_reward = - self.rwd_keys_wt['position'] * np.linalg.norm(else_error)
        reward_dict['position'] =  knee_error_reward +  else_error_reward
    ...
    def plot_draw(self):
        ...
        if self.flag_animation >= self.length_data - 1:
            ...
            if self.n == self.N:
                ...
                data_to_save_yaml = {
                    'x': self.x.copy(),
                    'qfrc_actuator_r': self.y_r.copy(),
                    'qfrc_actuator_l': self.y_l.copy()
                }
