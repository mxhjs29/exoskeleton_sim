...
class EnvCarry_WithoutExo(gym.Env, gym.utils.EzPickle, ObsVecDict):
    DEFAULT_CREDIT = """\
    ...
    def __init__(self, model_path, seed=None,env_credits=DEFAULT_CREDIT,max_episode_steps = 2500, n_stacks=4, **kwargs):
        ...
        # 观测关节
        self.obs_joints = ['pelvis_tx','ankle_angle_r', 'hip_flexion_l','ankle_angle_l']
    ...
    def step(self,a, **kwargs):
        ...
        a = (a + 1.) / 2.
        if self.count % 3 == 0 and self.flag_animation < self.length_data:
            # mocap的更新频率要慢一些
    ...
    def get_body_linear_vel(self):
        # mj_data.sensordata存放顺序是什么？pevils前面有43个body，速度则为43*3
        return  self.sim.data.sensordata[129: (129 + self.num_vel_limit)].reshape(self.num_bodies, 3).copy()
    def get_body_angular_vel(self):
        #为什么是2倍
        return self.sim.data.sensordata[self.num_vel_limit : 2*self.num_vel_limit].reshape(self.num_bodies, 3).copy()
    ...
    def get_obs_dict(self):
        ...
        obs_dict['proprioception'] = self.get_proprioception(obs_dict)
        return obs_dict
