...
def wxyz_to_xyzw(quat):
    # "Operate on the last dimension, but keep all the other dimensions as they are."
    return quat[...,[1,2,3,0]]
...
def quat_from_angle_axis(angle,axis):
    # Creates a quaternion that represents a rotation of a given angle (in radians) around a specified axis.
    theta = (angle / 2)[...,None]
...
def calc_heading(q):
    #Purpose: Calculates the heading angle (also known as yaw, or rotation around the vertical Z-axis) from a given orientation quaternion q.
...
def quat_to_tan_norm(q):
    # Converts a 4D quaternion into a 6D representation.
    # (0 0 1)(1 0 0)两个标准基做旋转
