...
data_path = "../data/imitation_data"
data_path_with_base_quat = "../data/imitation_data"
left_leg =  tuple([0,1,4,7,10])
right_leg =  tuple([0,2,5,8,11])
right_arm =  tuple([12,17,19,21,41])
left_arm =  tuple([12,16,18,20,26])
body =  tuple([0,12])
joints = ['pelvis_tx','lumbar_rotation']
actuators = ['addmagIsch_r']
bodys = ['SMPL_0', 'SMPL_1', 'SMPL_2', 'SMPL_4', 'SMPL_5', 'SMPL_7', 'SMPL_8', 'SMPL_10','SMPL_11',
            'SMPL_12','mocap_0', 'mocap_1', 'mocap_2', 'mocap_4', 'mocap_5', 'mocap_7', 'mocap_8', 'mocap_10','mocap_11',
...
for xml_id in range(xml_length):
    model_path = "../data/xml/myolegs/myolegs_collect_imitation.xml"
    ...
    with tqdm(total=number) as pbar:
        while index < number:
            ...
            for i in range(len(body) - 1):
                ...
                print(sim.data.xpos[bodys_id[body_child_left]].copy())
                print(sim.data.xpos[bodys_id[body_father_left]].copy())
                print(relative_body)
                print(dataset_body_norm)
            ...
            for i in range(N):
                ...
                print("flag_animation: ",flag_animation,"  i: ",i)
                ...
                q_frac_constraint = sim.data.qfrc_constraint[
                                    joints_id['pelvis_tx']:joints_id['lumbar_rotation'] + 1].copy()
                ...
                tau_truth = sim.data.qfrc_constraint[joints_id['pelvis_tx']:joints_id['lumbar_rotation'] + 1].copy() + \
                            sim.data.qfrc_passive[joints_id['pelvis_tx']:joints_id['lumbar_rotation'] + 1].copy() - \
                            sim.data.qfrc_bias[joints_id['pelvis_tx']:joints_id['lumbar_rotation'] + 1].copy()
                force_buffer.append(tau_truth)
                
                if (flag_animation > frame_all):
                    break
            ...
            obs_saved_dir = "../data/imitation_data"
            name = "bend_frq_2"
            ...
            initial_yaml_path = "../data/imitation_data/initial_state_with_exo.yaml"
...
# def compute_proprioception(input,) -> np.array:
