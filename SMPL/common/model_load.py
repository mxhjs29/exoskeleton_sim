...
from SMPL.common.quaternion import *
...
def obs_data_load(obs_true_path, index):
    ...
    # mission_id = data['task_id']
    # tau = data['joint_tau']
    # original_file = data['file_name_original']
    # muscle_data = data['muscle_activate']
    length = len(qpos)
    # return file, length, qpos, qvel, tau, people_id, mission_id, original_file.split('.')[0], muscle_data, len(muscle_data['DC'])
    return length, qpos, qvel
